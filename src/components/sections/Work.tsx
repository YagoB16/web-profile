...
function Work({ id }: SectionProps) {
    ...
    // Simula carregamento de dados dos projetos
    useEffect(() => {
    ...
    return (
        ...
        >
            ...
            {isLoading ? (
                <Skeleton className="h-4 w-96 mx-auto mt-5 mb-12" />
            ) : (
            ...
            <div className="grid grid-cols-[repeat(auto-fit,_minmax(200px,_1fr))] gap-6 my-10">
                {isLoading ? (
                    // Skeleton para os projetos
                    Array.from({ length: 4 }).map((_, index) => (
                        <div
                            key={index}
                            className="aspect-square rounded-lg relative w-50 max-sm:w-70 sm:w-full"
                        >
                            <Skeleton className="w-full h-full rounded-lg" />
                            <div className="bg-white w-10/12 rounded-md absolute bottom-5 left-1/2 -translate-x-1/2 p-5">
                                <div className="space-y-2">
                                    <Skeleton className="h-5 w-3/4" />
                                    <Skeleton className="h-4 w-full" />
                                </div>
                                <Skeleton className="w-6 h-6 rounded-full absolute right-5 top-1/2 -translate-y-1/2" />
                            </div>
                        </div>
                    ))
                ) : (
                    ...
                    workerData.map((project, index) => (
                        <div
                            ...
                            className="aspect-square bg-no-repeat bg-cover bg-center rounded-lg
                                relative cursor-pointer group w-50 max-sm:w-70 sm:w-full"
            ...
            {isLoading ? (
                <Skeleton className="w-40 h-12 rounded-full mx-auto my-20" />
            ) : (
